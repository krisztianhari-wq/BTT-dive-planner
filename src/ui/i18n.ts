...
const hu = {
  ...
  ratesNote: 'Leszállás 20 m/min, felszállás 9 m/min az első stopig, majd 3 m/min.',
  ...
  noStops: 'Nincs kötelező megálló. GUE „minimum deco" felszállás: 9 m/min, majd 3 m/min a felső 6 m-en, 6 m-en ajánlott 1–3 perc.',
  depth: 'Mélység (m)',
  minutes: 'Idő (perc)',
  runtimeCol: 'Runtime',
  gasPlan: 'Gázterv',
  ...
  msg: (m: Msg): string => {
    ...
    switch (m.code) {
      ...
      case 'ppo2AboveMax': return `A fenékgáz (${p.gas}) pO2 értéke ${p.ppo2} bar ${p.depth} m-en, meghaladja az 1.4 bar maximumot. MOD: ${p.mod} m.`;
      case 'ppo2AboveWorking': return `A fenékgáz (${p.gas}) pO2 értéke ${p.ppo2} bar, a GUE munkalimit 1.2 bar.`;
      case 'hypoxicBottomGas': return `A fenékgáz (${p.gas}) hipoxiás sekélyen (pO2 ${p.ppo2}).`;
      case 'decoSwitchExceedsMod': return `${p.gas} váltása ${p.depth} m-en meghaladja a MOD-ot (${p.mod} m, pO2 1.6).`;
      ...
      case 'backEndAboveLimit': return `END ${p.end} m a háti gázzal (${p.gas}), a GUE limit 30 m. Több hélium kell.`;
...
const en: Dict = {
  ...
  ratesNote: 'Descent 20 m/min, ascent 9 m/min to the first stop, then 3 m/min.',
  ...
  noStops: 'No mandatory stops. GUE "minimum deco" ascent: 9 m/min, then 3 m/min over the last 6 m, 1–3 min at 6 m recommended.',
  depth: 'Depth (m)',
  minutes: 'Time (min)',
  runtimeCol: 'Runtime',
  gasPlan: 'Gas plan',
  ...
  msg: (m: Msg): string => {
    ...
    switch (m.code) {
      ...
      case 'ppo2AboveMax': return `Bottom gas (${p.gas}) pO2 is ${p.ppo2} bar at ${p.depth} m, above the 1.4 bar maximum. MOD: ${p.mod} m.`;
      case 'ppo2AboveWorking': return `Bottom gas (${p.gas}) pO2 is ${p.ppo2} bar, the GUE working limit is 1.2 bar.`;
      case 'hypoxicBottomGas': return `Bottom gas (${p.gas}) is hypoxic when shallow (pO2 ${p.ppo2}).`;
      case 'decoSwitchExceedsMod': return `Switching to ${p.gas} at ${p.depth} m exceeds its MOD (${p.mod} m, pO2 1.6).`;
      ...
      case 'backEndAboveLimit': return `END ${p.end} m on the back gas (${p.gas}), the GUE limit is 30 m. More helium is needed.`;
