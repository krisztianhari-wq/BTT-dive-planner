...
export function PenPrintSheet({ t, u, lang, input, plan, events, agencyLabel, envLabel, flowLabel, evText, decoStages }: {
  ...
  return (
    <PrintFrame t={t} lang={lang} subtitle={t.printSubtitlePen(envLabel, agencyLabel, u)}>
      ...
      <section className="ps-half">
        <h3>{t.penPlan}</h3>
        <p className={`ps-verdict ${plan.overridden || !plan.feasible ? 'bad' : plan.unsupported ? 'warn' : 'ok'}`}>{plan.overridden ? `⚠ ${t.penBraveActive}` : !plan.feasible ? t.feasibleNo : plan.unsupported ? `⚠ ${t.penUnsupported(agencyLabel)}` : t.penFeasible}</p>
        <div className="ps-kpis">
